...
export default function AdminDashboardPage() {
  ...
  async function carregarDados() {
    try {
      ...
      if (!alunosResponse.ok) {
        throw new Error(`Erro ao buscar alunos: ${alunosResponse.status}`);
      }
      ...
      setFuncionarios(normalizarLista<Funcionario>(funcionariosData));

      if (receitasResponse.ok) {
        const receitasData = await receitasResponse.json();
        setReceitas(normalizarLista<Receita>(receitasData));
      }
    } catch (error) {
      console.error(error);
  ...
  const receitasPagas = useMemo(() => {
    return receitas.filter(
      (receita) => receita.status?.toLowerCase() === "pago",
    );
  }, [receitas]);

  const receitasAtrasadas = useMemo(() => {
    return receitas.filter(
      (receita) => receita.status?.toLowerCase() === "atrasado",
    );
  }, [receitas]);

  const receitaTotal = useMemo(() => {
    return receitasPagas.reduce(
      (total, receita) => total + numero(receita.valorPagamento),
      0,
    );
  }, [receitasPagas]);
  ...
  const receitaPorMes = useMemo(() => {
    const meses = new Map<string, { mes: string; valor: number }>();
    ...
    receitasPagas.forEach((receita) => {
      ...
      meses.set(chave, {
        mes: mes.replace(".", ""),
        valor: valorAtual + numero(receita.valorPagamento),
      });
    });
  ...
  const receitaPorPlano = useMemo(() => {
    ...
    receitasPagas.forEach((receita) => {
      ...
      planos.set(pagamento, valorAtual + numero(receita.valorPagamento));
    });
    ...
    return Array.from(planos.entries()).map(([plano, valor]) => ({
      plano,
      valor,
  ...
  async function adicionarFuncionario(e: React.FormEvent) {
    e.preventDefault();
  ...
  const stats = [
    {
      label: "Alunos ativos",
      valor: String(
        alunos.filter((aluno) => aluno.status?.toLowerCase() === "ativo")
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="mx-auto max-w-6xl px-4 py-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-white sm:text-2xl">
                {isAdm ? "Administrador" : "Painel do Funcionário"}
              </h1>

              <p className="text-sm text-zinc-500">
                {isAdm
                  ? "Painel administrativo · GymFlow"
                  : "Painel operacional · GymFlow"}
              </p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <p className="text-xs text-zinc-600">Atualização automática: 30s</p>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          ...
          >
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">
                      Plano {aluno.plano || "Não informado"} · desde{" "}
            ...
            {alunosFiltrados.length > 5 && (
              <button
                onClick={() => setMostrarTodosAlunos((valor) => !valor)}
          ...
          <SectionCard icon={UserCog} title="Equipe de Funcionários">
            <div className="flex flex-col divide-y divide-zinc-900">
              ...
              {funcionarios.map((funcionario) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-zinc-500">
                      {funcionario.cargo || "Cargo não informado"} · turno{" "}
                      {funcionario.turno || "Não informado"}
                    </p>
                  </div>
          ...
          <SectionCard icon={Mail} title="Sua Conta">
            <div className="flex flex-col divide-y divide-zinc-900">
              <div className="flex items-center justify-between py-2.5">
                ...
                <span className="text-sm font-medium text-zinc-200">
                  {isAdm ? "Administrador Geral" : "Funcionário"}
                </span>
              </div>
                ...
                <div className="lg:col-span-2">
                  ...
                  <div className="mt-6 flex items-center gap-2">
                    ...
                    <p className="text-sm text-zinc-400">
                      {receitasAtrasadas.length} pagamentos atrasados
                    </p>
                  </div>
                  ...
                  <div className="mt-3 flex flex-col divide-y divide-zinc-900">
                    ...
                    {receitas.map((receita) => (
                      ...
                      >
                        <div>
                          <p className="text-sm font-semibold text-white">
                            {receita.pagamento || "Pagamento"}
                          </p>

                          <p className="text-xs text-zinc-500">
                            {formatarMoeda(receita.valorPagamento)} · pagamento{" "}
                            {formatarData(receita.dataPagamento)}
                          </p>
                        </div>

                        {receita.status?.toLowerCase() === "atrasado" ? (
                          <button
                            onClick={() => marcarComoPago(receita.id)}
                ...
                <div>
                  ...
                  <div className="mt-6 flex items-center gap-2 rounded-lg border border-zinc-900 bg-zinc-900/40 p-3">
                    ...
                    <p className="text-xs text-zinc-400">
                      Dados calculados diretamente das receitas retornadas pelo
                      banco.
                    </p>
                  </div>
                </div>
        ...
        <div className="mt-6">
          <SectionCard icon={History} title="Status da conexão">
            <div className="flex items-center justify-between rounded-lg border border-zinc-900 bg-zinc-900/30 p-4">
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  Os dados são buscados pela API automaticamente.
                </p>
              </div>
